...
const int MIN_CONSECUTIVE_TO_TURN = 4;
const int MAX_CONSECUTIVE = 10;
const int DIRS = 4;
int dr[] = {-1, 0, 1, 0};
int dc[] = {0, 1, 0, -1};
...
int main() {
    ...
    for (int dir = 0; dir < DIRS; ++dir) {
        for (int max_c = 0; max_c <= MAX_CONSECUTIVE; ++max_c) {
            dist[dir][max_c].assign(n, vector<int>(m, INF));
        }
    }
    dist[1][0][0][0] = 0;
    dist[2][0][0][0] = 0;
    ...
    while (!pq.empty()) {
        ...
        for (int dir = 0; dir < DIRS; ++dir) {
            ...
            if ((cnt == MAX_CONSECUTIVE && dir == t) ||
                (cnt < MIN_CONSECUTIVE_TO_TURN && dir != t)) {
                continue;
            }
            int new_cnt = (dir == t ? cnt + 1 : 1);
            if (dir != t) assert(cnt >= MIN_CONSECUTIVE_TO_TURN);
    ...
    int res = INF;
    for (int dir = 0; dir < DIRS; ++dir) {
        for (int cnt = MIN_CONSECUTIVE_TO_TURN; cnt <= MAX_CONSECUTIVE; ++cnt) {
